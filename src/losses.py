...
class BaseLoss(torch.nn.Module):
    ...
    def __init__(self, min_N, max_N, dt):
        ...
        self.min_N = min_N
        self.max_N = max_N
        self.min_train_freq = 2 ** self.min_N # 真正的损失函数间隔，小的那部分
        self.max_train_freq = 2 ** self.max_N # 真正的损失函数间隔，大的那部分
        # sampling time
        self.dt = dt # (s)
...
class GyroLoss(BaseLoss):
    """Loss for low-frequency orientation increment
        该类指定了很多损失函数的计算方法，具体包括：
        1. 基于旋转矩阵的损失函数
        2. 基于四元数的损失函数，参考https://blog.csdn.net/benchuspx/article/details/108523498
        3. 基于旋转矩阵的损失函数（带有mask）
        4. 基于四元数的损失函数（带有mask）
        mask是为了给TUM数据集使用的
    """
